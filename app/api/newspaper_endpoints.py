...
MIN_ARTICLE_LENGTH = 200  # Minimum length for the article

@router.get("/api/long-random-article")
async def get_long_random_article():
  try:
    ...
    while attempts < 5:  # Limit to 5 attempts to find a valid article
      ...
      if len(random_article.text) >= MIN_ARTICLE_LENGTH:
        # Article details
        article_details = {
          "title": random_article.title,
          "author": random_article.authors,
...
# Endpoint to retrieve today's random article
@router.get("/api/today-random-article")
async def get_today_random_article():
  return await get_random_article()

@router.get("/api/word-from-today-random-article")
async def get_word_from_today_random_article():
  article_details = await get_random_article()
